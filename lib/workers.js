...
workers.validatCheckData = (originalCheckData) => {
    ...
    if (id && userPhone && protocol && url && method && successCodes && timeoutSeconds) {
        ...
        debug('Error: One of the checks is not properly formated. Skip it')
    }
}
...
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
    ...
    data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            ...
            debug('Error: Trying tp save update to one of the checks')
        }
    })
}
...
workers.alertUserToStatusChange = (newCheckData) => {
    ...
    helpers.sendTwilioSMS(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            debug('Success: User was alerted with the status to the change in the check')
        } else {
            debug('Error: Could not send sms to user who had a state change');
        }
    })
}
